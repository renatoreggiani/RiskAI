...
df_X = df.drop(columns = ['category', 'GSRAII Index'])
...
scores = cross_val_score(knn_m0, X, y, scoring='accuracy', cv=cv, n_jobs=-1)
...
df_X_final = drop_var_multicol(df_X)
...
scores = cross_val_score(knn_m1, X_s1, y, scoring='accuracy', cv=cv, n_jobs=-1)
...
scores = cross_val_score(knn_m2, X_s2, y, scoring='accuracy', cv=cv, n_jobs=-1)
...
scores = cross_val_score(knn_m3, X_s3, y, scoring='accuracy', cv=cv, n_jobs=-1)
